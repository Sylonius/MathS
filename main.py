...
"""01"""; def MultiplesOf3Or5(num=1000):
...
for i in range(num):
    if (i % 3 == 0 or i % 5 == 0):
        sum += i
return sum
"""02"""; def EvenFibonacciNumbers(self):
...
print(lst)
return lst[0]
"""04"""; def LargestPalindromeProduct(num):
...
"""05"""; def SmallestMultiple(num2, num1=1, increment=1):
...
while (True):  # janky do while loop
    ...
    if (didEvenlyDivide == False):  # condition
        ...
        break
return n
"""06"""; def SumSquareDifference(num1):
...
while (currentIndex < num1):
    ...
    if (MathProblems1to99.CheckIfPrime(currentNum)):
        currentIndex += 1
return currentNum
"""08"""; def LargestProductInASeries(seriesLength, _largeNum):
...
"""09"""; def SpecialPythagoreanTriple(num1):
...
for index in range(2, num1):
    if (MathProblems1to99.CheckIfPrime(index)):
        num += index
return (num)
"""11"""; def LargestProductInAGrid(_grid, _length=4):
...
"""12"""; def HighlyDivisibleTriangularNumber(_numFactors):
...
while (not done or index > 1000000):
    ...
    if (length > math.floor(_numFactors / 2)):
        ...
        return sum
"""13"""; def LargeSum(_numDigits, _str):
...
"""14"""; def LongestColatzSequence(_num):
...
for i in strNum:
    sumNum += (int)(i)
return sumNum
"""17"""; def NumberLetterCounts(_num):
...
return totalLength
"""18"""; def MaximumPathSumOne(_str):
"""Given a string that is a triangle in shape, this function will find the greatest path"""
gridHeight = 15
pyramid = MathProblems1to99.ParseStrToGrid(gridHeight, gridHeight, _str)
# print(pyramid)
...
# return(grid[0,0])
return (grid)
"""19"""; def CountingSundays(self):
pass
"""20"""; def FactorialDigitSum(_num):
"""Adds the digits of the factorial of _num"""
nums = CommonlyUsedFunctions.factorial(_num)
numsStr = str(nums)
total = 0
for i in numsStr:
    total += int(i)
return total
"""21"""; def AmicableNumbers(self, _num):
listOfPairs = []
for i in range(1, _num + 1):
    divisors = MathProblems1to99.GetProperDivisors(i)
    divisorSum = 0
    for j in divisors:
        divisorSum += j
...
return totalValue
"""23"""; def NonAbundantSums(self):
abundantList = []
for i in range(11, 28124):
    tempList = MathProblems1to99.GetProperDivisors(i)
    listTotal = 0
...
for checkNums in range(1, 28124):
    ...
    if numCantBeWritten:
        tempTotal += checkNums
return tempTotal
"""24"""; def LexicographicPermutations(self, _size):
myList = MathProblems1to99.AllPermutations(_size)
return myList[999999]
"""25"""; def ThousandDigitFibonacciNumber(self, _digits):
...
while (fibboLen < _digits and attempts < 1000000):
    attempts += 1
    checkTuple = MathProblems1to99.FibonacciHelper(checkTuple)
    fibboLen = len(str(checkTuple[1]))
return attempts + 2  # to account for starting tuple add 2
"""26"""; def ReciprocalCycles(self, _size):
...
for n in range(2, _size + 1):
    ...
    k = 0
    primeFactors = MathProblems1to99.PrimeFactorisation(n)
...
"""27"""; def QuadraticPrimes(self, _size):
...
for a in range(-_size + 1, _size + 2):
    for b in range(-_size, _size + 1):
        ...
        while stillConsecutive:
            isPrime = MathProblems1to99.CheckIfPrime(abs(n ** 2 + a * n + b))
            if isPrime:
                n += 1
                consecutiveTerms += 1
...
"""28"""; def NumberSprialDiagonals(_radius):
total = 0
for i in range(0, 4):
...
class HelperFunctions:
    ...
    def GetProperDivisors(_num):
        factors = MathProblems1to99.GetFactors(_num)
        pureList = []
        for i in factors:
            for j in range(2):
                pureList.append(i[j])
    ...
    def AllPermutationsHelper(prefix, remaining, results):  # AI Generated
        ...
        for i in range(len(remaining)):
            MathProblems1to99.AllPermutationsHelper(
                prefix + remaining[i],
    ...
    def AllPermutations(_numberOfItems):
        ...
        results = []
        MathProblems1to99.AllPermutationsHelper('', digits, results)
        return results
...
if __name__ == '__main__':
    ...
    math1To99 = MathProblems1to99()
    myResult = math1To99.DistinctPowers(100,100)
    print(myResult)
